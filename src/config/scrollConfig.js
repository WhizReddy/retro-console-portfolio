...
export const defaultScrollConfig = {
  thresholds: {
    ...
    developerInfo: 100,       // Show developer introduction
    monitorGuidance: 200,     // Guide to monitor
  },
  timings: {
  ...
  messages: {
    ...
    introduction: "Welcome to my retro studio. Scroll slowly & interact with objects to discover my work.",
...
export function createScrollConfig(userConfig = {}) {
  ...
  for (let i = 1; i < thresholds.length; i++) {
    if (thresholds[i] <= thresholds[i - 1]) {
      console.warn('Scroll thresholds should be in ascending order. Using defaults.');
